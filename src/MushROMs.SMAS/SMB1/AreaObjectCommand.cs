...
namespace MushROMs.SMAS.SMB1
{
    public struct AreaObjectCommand
    {
        ...
        {
            get => ((YInternal ? Value3 : Value2) & 0x80) != 0;
            set
            {
                if (YInternal)
                {
        ...
        public override int GetHashCode() =>
            (Value1) | (Value2 << 8) | (Value3 << 16);

        public override string ToString() =>
            SR.GetString("({0}, {1}): {2}", X.ToString("X"), Y.ToString("X"), AreaObjectCode);
    }
}
